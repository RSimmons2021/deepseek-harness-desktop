...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | Projects the authorization seam onto the generated Remote namespace as a poll: a sign-in settles on a person's schedule, so the attempt outlives the call that starts it and the page follows it through poll. |
