...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | 把 authorization seam 以轮询方式投影到生成的 Remote namespace：登录按人的节奏完成，因此该 attempt 比启动它的调用活得更久，页面通过 poll 跟进。 |
