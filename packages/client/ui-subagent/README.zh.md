...
委派是唯一一件无需读者动作就会发生的事，因此谱系会把它显示出来，而不只是记录下来。只要还有被计入的后代在运行，页头触发器就会从它的活动圆点向外发出一圈涟漪，节奏与下方分支的绘入一致，颜色也与 ongoing 圆点相同 —— 一种色调在任何地方都表示「正在运行」。在树中，每条分支按其位置错峰绘入，因此六路 fan-out 读起来是六件事在启动，而不是一整块凭空出现；错峰在第六条处封顶，再往后最末一条的到达就会变成等待而不是效果。正在运行的子树，其连接导轨会提亮并有一道高光沿它向下移动，因为导轨是父级与它所启动之物之间唯一的连线。在 `prefers-reduced-motion` 下，事实保持不变 —— 运行中的导轨保留它的颜色，每条分支也都在 —— 只有承载它们的那部分移动消失。
