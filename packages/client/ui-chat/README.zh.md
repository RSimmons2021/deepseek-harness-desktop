...
Details column 绘制 `--dsh-details-surface`，缺省回退到普通的不透明 base，因此自行在其背后绘制底色的宿主可以把它设为 `transparent`。不设置时行为不变。
...
「设置 → 通用设置」在同一命名空间中还提供第二项持久化偏好：对话中展示多少模型的思考内容。`仅摘要` 让每个 Think 行保持关闭直到被打开，也就是此前的行为。`思考时展开`（默认）会在模型仍在产出该推理块期间保持它展开，并在正文开始时关闭，因此思考内容在发生的当下就可读，而不会在事后留下一整面墙；正在到达且已展开的正文有高度上限并跟随自身尾部，因此再长的思考也不会把正文顶出屏幕。`始终展开` 让每个推理块保持打开。点击对它所落在的那一行始终优先于偏好，而该覆盖会在该块停止运行后被遗忘，因此一次刻意的查看不会把某个块钉住展开到 transcript 结束。
