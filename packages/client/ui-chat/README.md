...
Settings → General exposes a second persisted preference in the same namespace: how much of the model's reasoning the transcript shows. `Summary only` keeps each Think row shut until it is opened, which is the historical behavior. `While it thinks` — the default — opens a reasoning block for as long as the model is still producing it and shuts it once the answer begins, so the thinking is readable while it happens without leaving a wall of it behind; an open, still-arriving body is bounded and follows its own tail, so a long think never pushes the answer off screen. `Always open` keeps every reasoning block expanded. A click always wins over the preference for the row it lands on, and that override is forgotten once the block stops running, so one deliberate peek does not pin a block open for the rest of the transcript.
