...
import type { ModelsAuthorization, ModelsSettingsStore, ProviderRow } from './store.ts'
...
export interface ModelsSectionInjected {
  ...
  /**
   * Authorization Remote face for the sign-in card. It drives a flow rather
   * than reading or writing settings, so it is not a settings-shaped callback.
   */
...
export function ModelsSection(props: ModelsSectionProps): ReactNode {
  ...
  return (
    <Loaded
      injected={{ controller, useSnapshot, operations, authorization, schema, t }}
...
function Loaded({ injected, renderSlot }: { injected: ModelsSectionFace; renderSlot: ModelsRenderSlot }): ReactNode {
  ...
  const adoptSignedInRoute = async (route: string): Promise<void> => {
    ...
    const discovered = await operations.discoverModels('llm-pi-ai', { provider: route })
    const models = discovered.kind === 'found' ? discovered.models.map(model => ({ id: model.id })) : []
    const profile = models.length === 0 ? {} : { models }
    await operations.writeSettings(
      'llm-pi-ai',
