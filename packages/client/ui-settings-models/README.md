...
The Models page also lists the providers that ship an interactive login. Subscription grants lead the list, because the rest of the catalog offers a login too and an unsorted list buries the plans a person already holds. A committed sign-in declares the route it belongs to, with whatever models the adapter can discover for it, unless a route already exists. A route that declares no models registers and serves, but the selector shows nothing for it — the catalog is whatever the settings document says — so a signed-in provider with a bare route looks broken while being perfectly healthy. The adapter runs the routes the settings document declares and is mounted with none, so a grant on its own leaves the provider invisible — the credential is stored and nothing reads it. The declared route names no credential reference, which is what defers it to the adapter's own discovery, where the grant just landed. A running attempt renders inside the row it was started from: the flow's first question is usually which login method to run, and a catalog this long put that question far below the fold, where a sign-in looked like it had simply stalled.
