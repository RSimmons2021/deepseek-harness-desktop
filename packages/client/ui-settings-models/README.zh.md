...
Models 页面还会列出提供交互式登录的 provider。订阅类授权排在最前，因为目录中其余 provider 同样提供登录，未排序的列表会淹没用户已经持有的订阅。完成的登录会声明其所属 route，并附带 adapter 能发现的 model（若该 route 尚不存在）。不声明任何 model 的 route 仍会注册并服务请求，但选择器不会显示它 —— catalog 完全取决于 settings 文档 —— 因此已登录却只有空 route 的 provider 看起来像是坏了，实际上完全正常。adapter 只运行 settings 文档声明的 route，且挂载时没有任何 route，因此仅有授权会让 provider 不可见 —— 凭据已保存却无人读取。所声明的 route 不指定凭据引用，从而交由 adapter 自身的凭据发现处理，而授权正是落在那里。进行中的尝试渲染在其发起所在的行内：flow 的第一个问题通常是选择登录方式，而这么长的目录会把该问题推到视野之外，使登录看起来像是卡住了。
