...
Models 页面还会列出提供交互式登录的 provider。订阅类授权排在最前，因为目录中其余 provider 同样提供登录，未排序的列表会淹没用户已经持有的订阅。进行中的尝试渲染在其发起所在的行内：flow 的第一个问题通常是选择登录方式，而这么长的目录会把该问题推到视野之外，使登录看起来像是卡住了。
