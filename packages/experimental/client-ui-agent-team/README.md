...
While the surface is open it holds one bounded `agentTeams/waitForChange` call, reloads the whole view on every observed change, and re-enters the wait; a transport failure ends the loop and leaves the manual refresh as the way back. As the desktop surface the workspace toolbar also carries an appearance control beside the motion pause: it reads the resolved scheme from the theme runtime and writes the opposite one as the durable preference, so it never becomes a second source of theme state. A running teammate card carries an interrupt control beside the card button — never inside it, because the card button is the open-teammate target and a button cannot nest.
