...
describe('Agent Teams profile invariant companion', () => {
  it('registers an explained empty installer and removes it on disposal', async () => {
    ...
    const fiber = ctx.plugin(ProfileInvariant)
    await fiber.await()
    expect(registered).toEqual(['@deepseek-ai/dsh-experimental-agent-team-profile'])
    expect(ProfileInvariant.inject).toEqual(['invariants'])
    await fiber.dispose()
  })
})
