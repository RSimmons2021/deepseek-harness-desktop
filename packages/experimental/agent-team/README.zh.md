...
`TeamService` 除了 roster、mailbox、task 与 lifecycle operation，还直接负责生成式 `agentTeams/view`、`agentTeams/createTask`、`agentTeams/updateTask`、`agentTeams/spawnTeammate`、`agentTeams/sendMessage`、`agentTeams/interrupt`、`agentTeams/follow`、`agentTeams/activity` 与 `agentTeams/tail` Remote method。`follow` 是浏览器的实时更新接缝：一个逻辑 stream，先给出当前 view，之后每观察到一次变化再给出一次 view。每个 frame 承载完整 view 而非增量，因为本 Team 的 view 很小，而在每个 frame 上整体替换的 client 不会发生漂移。stream carrier 拥有 cancellation，因此断开连接的浏览器会立即结束其等待，而不是留下一个 wait 直到超时。service 自身的 `waitForChange` 仍为 model-facing tool 保留 —— 该 tool 自带 signal。Team 通过 **role（角色）** 来配置成员。一个 role 携带名称词干、简短的职责说明、常驻 brief、context 模式，以及可选的 route —— 该角色的成员所运行的 provider、model 与 reasoning effort。创建一位成员就是指定一个 role 并描述工作；Team 会从该 role 的词干派生出一个空闲名称，因此第二位 reviewer 叫 `reviewer-2`，而不是撞上一个调用方从未选择过的名字。`Config.roles` 会整体替换内置集合，因为一个部署拥有哪些模型、以及哪类工作配得上其中哪一个，都是部署事实。**没有任何内置 role 指定模型**：未做任何配置的 Team 完全运行在 Lead 的 route 上，因此路由在默认情况下绝不以质量为代价。role 的 route 被记录在持久化的成员行上，而不是从存活的 Agent 读取，因为处于空闲的成员仍然运行在它被创建时的 route 上 —— 只读存活 Agent 会让它报告成 Lead 的模型。不指定 role 的调用方需要自行组装成员，并且必须提供 name 与 description；未声明的 context 模式为 `fresh`。

每个 roster row 都携带 `pendingMessages` —— 发给该成员、但它尚未记录的持久消息数：安静消息不会启动空闲成员，因此在两轮之间发出的消息会停在它的 mailbox 里，而此前唯一的迹象只是 `sendMessage` 返回 `queued` 而非 `accepted`。当成员处于 attached 状态且 projection 有答案时，每个 row 还会携带 `effort`：轮次、模型与工具的墙钟时间，以及 provider 的输入、输出与缓存命中 token —— 它们读取该成员自身的 `sessionStats` 与 `tokenUsage` projection，而不在此处折叠。`activity` 直接读取 Lead Session 日志本身，而不另建一份记录，因此它返回的内容不会与 Team 实际发生的事情相矛盾；它按最新在前返回条目，数量由调用方给出的 limit 限定在 1 到 200 之间，并且与 fold 一样跳过 fork 从祖先 Team 继承的 Team 记录。其条目只携带结构化事实——记录的主体与其到达的状态——而非组合好的句子，因为为这些事实命名的文案由渲染它的界面按 locale 拥有。`tail` 读取某个具名成员自身已 attach 的 Session 日志，取其最近记录的行 —— 它说的话、它运行的 tool，以及这些 tool 返回了什么 —— 按最新在前返回，数量由调用方 limit 限定在 1 到 50 之间，每行还按 400 个单元截断，因为 tail 是用来观察成员工作的，而不是用来重建它的完整记录。运行时不再持有的成员没有可 tail 的 live 日志，因此返回空。`waitForChange` 也会被任意成员 Session 中的这三类事件释放，因此跟随 Team 的界面会在它本就等待的信号上收到成员的工作；流式输出的成员每个 step 释放它数次，而不是每个 token 一次。`sendMessage` 同样可从浏览器调用：其 Remote request 以单个字符串携带消息正文，由 service 封装为 content block，且不携带 cancellation —— enqueue 在尝试投递前即已持久化，不存在可取消的入队前窗口。`spawnTeammate` 则可从浏览器调用：其 Remote request 既不携带 signal 也不携带 provider，因为 provider 是本包通过 `freshProvider`/`forkProvider` 拥有的部署选择，并由请求的 context mode 解析。Model-facing spawn tool 读取同一个 `providerFor`，因此 tool 与浏览器不会对 `fresh` 与 `fork` 的启动方式产生分歧。`./remote` 导出由 Web UI 挂载的 Client contribution，`./client` 则重新导出可在浏览器 compilation face 中安全使用的 request、view 与 task mutation result type。Typert 在外层 `RemoteResult` 中保留 transport failure；create 与 update rejection 则作为 transport 成功响应中的显式 domain result，其中过期的 update revision 会区分为 task conflict。
...
- **除文件系统 tool 外，write scope 仅作提示**——[`dsh-experimental-agent-team-write-lease`](../agent-team-write-lease/README.zh.md) 会拒绝成员对他人进行中任务所声明范围的 `fs` tool 写入，`writeRefusal` 就是它询问的决策。Bash、formatter、代码生成器与直接外部写入仍会绕过它，正如它们绕过文件版本检查一样；Lead 必须协调 owner 并检查最终 diff。
...
- **成员投入仅在成员处于 attached 状态时报告**——`TeamMemberView.effort` 读取该成员自身的 `sessionStats` 与 `tokenUsage` projection，这需要其 live Session。teammate 在两轮之间并未 attached，因此不会报告它上一次的投入，而是什么都不报告。读取冷成员的 checkpoint 需要其 Session header，而从 Session id 得到 header 的每条路径要么是异步的，要么会读取整份日志——同步的 roster 读取不允许这样做。
