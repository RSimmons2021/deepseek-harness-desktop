...
// Imported for their `sessionStats` and `tokenUsage` projection-key merges,
// which these reads depend on. Token-meter's root re-exports its projection
// modules; session-stats declares its key in the projection module itself, so
// that module is what carries it.
import type {} from '@deepseek-ai/dsh-session-stats/src/projection.ts'
...
export class TeamRoster {
  ...
  private effortOf(session: Session): TeamMemberEffort | undefined {
    ...
    return {
      ...
      inputTokens: usage?.totals.uncachedInputTokens ?? 0,
      outputTokens: usage?.totals.outputTokens ?? 0,
      cacheReadTokens: usage?.totals.cacheReadTokens ?? 0,
    }
  }
