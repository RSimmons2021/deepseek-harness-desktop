 ...
 *
 * The projected state resolves the ids an event carries into the names a person
 ...
 * @param state - the Team state the log has produced so far.
...
export function activityOf(
  ...
  // A fork inherits the ancestor Team's records. The projection ignores them,
  // so the history must too, or a forked Lead reads another Team's past as its
  // own.
  if (event.data.teamId !== state.id) return undefined
  const at = { seq: event.seq, time: event.time }
  // The projected state stores its rows as arrays so it can be persisted, so a
  // lookup is a scan rather than a map read; a Team is bounded by its own
  // member and message limits, which is what keeps that honest.
  const memberName = (id: SessionId): string =>
    state.members.find(member => member.id === id)?.name ?? String(id)
  switch (event.type) {
    case 'team/member': {
      const member = event.data.member
    ...
    case 'team/message/queued': {
      ...
      return {
        ...
        target: memberName(message.targetId),
      }
    }
    case 'team/message/delivered': {
      // The queued record carries the sender, so a delivery whose enqueue is no
      // longer in the projection names the target it reached and nothing else.
      const message = state.messages.find(candidate => candidate.id === event.data.messageId)
      return {
        ...at,
        kind: 'message-delivered',
        subject: message?.senderName ?? '',
        target: memberName(event.data.targetId),
      }
    }
    /* v8 ignore next 2 -- TeamEventType is closed and every member is handled above. */
    default:
