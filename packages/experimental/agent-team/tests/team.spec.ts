...
describe('Team Remote API', () => {
  it('exports Team views and task mutations from the owning service', async () => {
    ...
    expect(ctx.agentTeams.remoteView(lead)).toEqual({
      ...
      tasks: [],
      // The surface offering to add a teammate needs the limit the service enforces.
      capacity: 8,
    })
  ...
  it('frames a Remote peer message as text and preserves send rejections', async () => {
    ...
    const request = { target: 'editor', message: 'ship it' as const }
  ...
  it('counts the mail a member has not taken, and wakes an inactive one rather than queuing', async () => {
    const { ctx, lead } = await setup(['hang'])
    const editor = await spawn(ctx, lead, 'editor')
    ...
    // Peer messages steer: one sent to a member that is not running starts it
    // rather than sitting in the mailbox, so nothing accumulates behind it.
    ctx.agentTeams.interrupt(lead, 'editor')
    await waitNoAgent(ctx, editor.member.id)
    const sent = await ctx.agentTeams.sendMessage(lead, {
      target: 'editor', content: content('take the next one'), signal: SIGNAL,
    })
    expect(sent.status).toBe('accepted')

    // The count is what a message that could NOT be taken would show; once the
    // delivery is recorded there is nothing outstanding for either member.
    await vi.waitFor(() => {
      const members = ctx.agentTeams.remoteView(lead).members
      expect(members[1]).toMatchObject({ name: 'editor', pendingMessages: 0 })
  ...
  it('reports what each member has spent, and nothing without the projections', async () => {
    const { ctx, lead } = await setup(['hang'])
    // The composition mounts the projection registry, but this Lead has run no
    // turn, so neither unit has state: no effort, rather than zeros that would
    // read as a member which ran and spent nothing.
    expect(ctx.agentTeams.remoteView(lead).members[0]?.effort).toBeUndefined()

    // Token usage keeps its buckets under `totals`, so the fixture mirrors the
    // unit's own state rather than the flat shape it once had.
    const states: Record<string, unknown> = {
      sessionStats: { turns: 3, steps: 7, llmMs: 12_400, toolMs: 1100 },
      tokenUsage: {
        totals: { uncachedInputTokens: 18_200, outputTokens: 2400, cacheReadTokens: 96_000, cacheWriteTokens: 0 },
        last: null,
      },
    }
    const projections = ctx.get('sessionProjections')
    if (projections === undefined) throw new Error('the projection registry is not mounted')
    // Bound with its real answer type: read through the registry's own generic
    // face it is void here, and the stub has to return what it read.
    const realStateOf = projections.stateOf.bind(projections) as (session: never, key: string) => unknown
    vi.spyOn(projections, 'stateOf').mockImplementation(
      ((session: never, key: string) => key in states ? states[key] : realStateOf(session, key)) as never,
    )
    ...
    states['tokenUsage'] = {
      totals: { uncachedInputTokens: 5, outputTokens: 6, cacheReadTokens: 7, cacheWriteTokens: 0 },
      last: null,
    }
