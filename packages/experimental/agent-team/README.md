...
`TeamService` owns the generated `agentTeams/view`, `agentTeams/createTask`, `agentTeams/updateTask`, `agentTeams/spawnTeammate`, `agentTeams/sendMessage`, `agentTeams/interrupt`, `agentTeams/follow`, `agentTeams/activity`, and `agentTeams/tail` Remote methods beside the roster, mailbox, task, and lifecycle operations. `follow` is the browser's live-update seam: a logical stream yielding the current view, then the view again after every observed change. Every frame carries the whole view rather than an increment, because this Team's view is small and a client that replaces it on each frame cannot drift. The stream carrier owns the cancellation, so a browser that disconnects ends its wait immediately rather than leaving one outstanding until a timeout expires. The service's own `waitForChange` remains for the model-facing tool, which supplies its own signal. A Team is staffed from **roles**. A role carries a name stem, a short responsibility, a standing brief, a context mode, and optionally a route — the provider, model, and reasoning effort its teammates run on. Creating a teammate is naming a role and describing the work; the Team derives a free name from the role's stem, so a second reviewer is `reviewer-2` rather than a collision the caller never chose. `Config.roles` replaces the built-in set, because which models a deployment has and which of them a kind of work deserves are deployment facts. **No built-in role names a model**: a Team that configures none runs entirely on the Lead's route, so routing never costs quality by default. A role's route is recorded on the durable member row rather than read from the live Agent, because an idle teammate still runs on the route it was created with — reading only the live Agent reported the Lead's model for it. A caller with no role composes a teammate by hand and must supply a name and a description; the context mode it does not state is `fresh`.

Each roster row carries `pendingMessages`, the durable messages addressed to that member which it has not recorded yet: a quiet message does not start an idle member, so one sent between turns waits in its mailbox, and `sendMessage` answering `queued` rather than `accepted` was previously the only sign of it. Each row also carries `effort` when its member is attached and a projection answers: turns, model and tool wall time, and provider input, output, and cache-read tokens, read from that member's own `sessionStats` and `tokenUsage` projections rather than folded here. `activity` reads the Lead Session log itself rather than a second record, so nothing it returns can disagree with what the Team did; it returns the newest entries first, bounded by a caller-supplied limit from one through two hundred, and skips Team records a fork inherited from an ancestor Team exactly as the fold does. Its entries carry structured facts — the recorded subject and the state it reached — and never a composed sentence, because the copy naming them is locale-owned by the surface that renders them. `tail` reads one named member's own attached Session log for the lines it recorded most recently — its prose, the tools it ran, and what they returned — newest first, bounded by a caller limit from one through fifty and by a four-hundred-unit cut per line, because a tail is for watching a member work rather than reconstructing its transcript. A member the runtime is not holding has no live log to tail and answers with nothing. `waitForChange` is released by those same three event types in any member's Session, so a surface following the Team sees a member's work arrive on the signal it already waits on, and a streaming member releases it a few times per step rather than once per token. `sendMessage` is reachable too: its Remote request carries the message as one string that the service frames as a content block, and no cancellation, because the enqueue is durable before delivery is attempted and leaves no pre-queue window to cancel. `spawnTeammate` is reachable: its Remote request carries no signal and no provider, because the provider is a deployment choice this package owns through `freshProvider`/`forkProvider` and resolves from the requested context mode. The model-facing spawn tool reads the same `providerFor`, so the tool and the browser cannot disagree about what `fresh` and `fork` start. The `./remote` export supplies the Client contribution mounted by the Web UI, while `./client` re-exports the request, view, and task-mutation result types that are safe in a browser compilation face. Typert retains transport failures in its outer `RemoteResult`; create and update rejections remain explicit domain results inside a successful transport response, with stale update revisions distinguished as task conflicts.
...
- **Member effort is reported only while a member is attached** — `TeamMemberView.effort` reads the member's own `sessionStats` and `tokenUsage` projections, which needs its live Session. A teammate between turns is not attached, so it reports nothing rather than what it last spent. Reading a cold member's checkpoint needs its Session header, and every path to one from a Session id is asynchronous or reads the whole log, which a synchronous roster read may not do.
