...
**Animate card widths directly.** Rejected because width tweens trigger layout on every frame and cannot preserve sibling continuity when hover retargets rapidly. Shared-layout projection measures the new flex layout once and animates compositor transforms.
