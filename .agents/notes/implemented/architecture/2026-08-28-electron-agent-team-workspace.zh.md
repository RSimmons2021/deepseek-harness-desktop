...
Electron 在任何页面加载前会向 renderer user agent 追加 `DeepSeekHarnessDesktop` 标记。Layout plugin 使用这个 assembly-time 标记注册最小化 [`DesktopFrame`](../../../../packages/client/ui-layout/src/client/DesktopFrame.tsx)，而不是浏览器 `AppFrame`。该 frame 把窗口布局为会话侧栏、Team workspace、conversation 与 details，并在其上渲染 frame 级 overlay layer，使桌面既保留 Team workspace 作为主 surface，也能触达对话、会话历史、设置与工具详情。它使用与浏览器 frame 相同的 panel store 与 `ctx.layout` 挂接。渲染 sidebar 同时会带入 settings onboarding，因此全新的桌面 profile 现在会先显示测试须知与 API key 提示，而不再跳过它们。Team surface 在浏览器中是 dialog，在这里是 column，因此在 `standalone` 下其 panel 在宿主列内正常流动，而不再覆盖整个视口。Sidebar 与 details slot 仍保持声明以便普通 Client plugin 激活，但没有组件渲染它们。Agent Teams Client 占据该 seat，选择现有 Session 或创建一个 Session，并立即挂载 Team Alpha workspace。浏览器 sidebar、conversation hero、settings onboarding 与 testing notice 均不会被渲染，而不是在首次绘制后再隐藏。
...
Team 调色板跟随当前主题。Light 为基础，presenter 的 dark 属性整体替换它，但仅有四个锚点因主题而异：ground、ink、阴影权重与 ambient 强度。其余 Team token 均由它们派生，因此 stroke、card wash 与 sheen 会随主题变化，而不需要维护两套值。Ink 透明度是例外，按主题分别设定：在暗色上作为柔和标签的白色比例，放到浅色底上会失去对比，因此 light 使用更重的 muted、faint 与 stroke 值。作为应用 surface，ambient 是窗口的底色而非 workspace column 的装饰：它覆盖整个视口，conversation column 把 `--dsh-conversation-surface` 接缝设为 transparent 以让其透出，frame 则提供纯色半透明 scrim 保证对比度，而不使用会让窗口每帧重新混合的 backdrop blur。Workspace toolbar 在动画暂停按钮旁提供外观切换控件，它读取已解析的 scheme 并把相反值写为持久偏好，自身不持有主题状态。桌面 host 同时通过 `nativeTheme.themeSource` 请求暗色 renderer；macOS 与 Windows 会将其带入 `prefers-color-scheme`，Linux 不会，因此在 Linux 上该 surface 会跟随桌面会话，直到在应用设置中显式设置主题偏好。
